...
int chessBoard::getNextGameId(database &current) {
...
void chessBoard::parseFile(string filename) {
	...
	while(infile.good()) {
		getline(infile, move);
		if (blankCellCount == 0 && move.find("[White ") != string::npos)
			blankCellCount = 1;
		else if (blankCellCount == 1 && move.find("[Black ") != string::npos)
			blankCellCount = 2;
		else if (blankCellCount == 2 && move.find("[Result ") != string::npos)
			blankCellCount = 3;
...
string chessBoard::fenGenerator(){
	...
	vector<vector<char> > layout(NO_OF_RANKS,vector<char>(NO_OF_FILES));
	...
	for ( it = configuration.begin() ; it != configuration.end() ; it++){
			...
			if ( it->second.getColor() == 'w')
				layout[row-1][col] = p_type.at(0);
			if ( it->second.getColor() == 'b')
				layout[row-1][col] = tolower(p_type.at(0));
	...
	for ( int i = NO_OF_RANKS - 1 ; i >= 0 ; i--){
		...
		int blankCellCount = 0;
		for ( int j = 0 ; j < NO_OF_FILES ; j++){
			if ( layout[i][j] == '0' && inBlank == false){
				inBlank = true;
				blankCellCount++;
			}
			else if ( layout[i][j] == '0' && inBlank == true){
				blankCellCount++;
			}
			else if ( layout[i][j] != '0' && inBlank == false)
				FEN_state += layout[i][j];
			else if ( layout[i][j] != '0' && inBlank == true){
				string blank_blankCellCount = intToString(blankCellCount);
				FEN_state += blank_blankCellCount;
				FEN_state += layout[i][j];
				blankCellCount = 0;
				inBlank = false;
			}
	...
	if (parsed_move.type[0] == 'P') {
		if (parsed_move.is_capture)
			if (is_enpassant(destination)) {
                new_dest.row = new_dest.row + 2 * ( ( ('w' - turn_char)/('w' - 'b') ) - 0.5 ) * PREVIOUS_ROW ; 
			/*	if (turn_char  == 'b')
					new_dest.row = new_dest.row + PREVIOUS_ROW;
				else 
					new_dest.row = new_dest.row - PREVIOUS_ROW;
		*/	}
	}
...
string chessBoard::fetch_conflict(string move) {
		int length = move.length();
