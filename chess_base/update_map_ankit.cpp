...
bool is_enpassant(map <string, Piece> piece_map, cell destination);
...
map <string, Piece> insert_promoted_piece(map <string, Piece> piece_map, char promote_type, cell destination, char promote_color);
void print_board1(map<string, Piece>);
...
int main(){
    ...
    piece_map = update_map (piece_map, "d4", 1, en_passant);
    piece_map = update_map (piece_map, "e6", 2, en_passant);
    piece_map = update_map (piece_map, "Na3", 3, en_passant);
    piece_map = update_map (piece_map, "g5", 4, en_passant);


    piece_map = update_map (piece_map, "Bd2", 5, en_passant);
    piece_map = update_map (piece_map, "g4", 6, en_passant);
    piece_map = update_map (piece_map, "f4", 7, en_passant);
    piece_map = update_map (piece_map, "gxf3", 8, en_passant);
    piece_map = update_map (piece_map, "g4", 9, en_passant);
    piece_map = update_map (piece_map, "e5", 10, en_passant);
    piece_map = update_map (piece_map, "g5", 11, en_passant);
    piece_map = update_map (piece_map, "Nh6", 12, en_passant);
    piece_map = update_map (piece_map, "g6", 13, en_passant);
    piece_map = update_map (piece_map, "f5", 14, en_passant);
    piece_map = update_map (piece_map, "g7", 15, en_passant);
    piece_map = update_map (piece_map, "d6", 16, en_passant);
    piece_map = update_map (piece_map, "gxh8=Q", 17, en_passant);
...
data_from_input convert_str2data(string move, int turn) {
    ...
    chpiece.is_castling = false;
    chpiece.is_chkmate = false;
	chpiece.conflict = 'n';
	chpiece.is_capture = false;
    size_t found;
...
bool is_enpassant(map <string, Piece> piece_map, cell destination) {
	map <string, Piece>::iterator it;
	for (it = piece_map.begin(); it != piece_map.end(); it++)
		if ((*it).second.row == destination.row && (*it).second.column == destination.column)
			return false;
	return true;
}
...
map <string, Piece> insert_promoted_piece(map <string, Piece> piece_map, char promote_type, cell destination, char promote_color) {
	...
	if (it == piece_map.end()) {
		...
		promoted_piece.type += max_index;
		piece_map[reference_string] = promoted_piece;
	}
	else it->second = promoted_piece;
	return piece_map;
}

void print_board1(map<string,Piece> init_map) {
		for (int i = 8; i > 0; i--) {
				cout << ":";
				for (char a = 'a'; a <= 'h'; a++) {
						map<string, Piece>::iterator it = init_map.begin();
						while(it != init_map.end()) {
								if ((*it).second.row == i && (*it).second.column == a && (*it).second.isAlive) {
										cout << (*it).second.type << (*it).second.color<< ":";
										break;
								}
								it++;
						}
